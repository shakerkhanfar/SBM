...
export default function VoiceAgentPage() {
  ...
  const { status, micMuted, startAgent, stopAgent, toggleMicMute } = useVoiceAgent({
    ...
    onTranscriptionReceived: (text) => {
      setTranscriptions((prev) => [...prev, { speaker: 'User', text, timestamp: new Date() }]);
    },
    onAnswerReceived: (text) => {
      setTranscriptions((prev) => [...prev, { speaker: 'Agent', text, timestamp: new Date() }]);
    },
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-center gap-3">
              {!isActive ? (
                ...
                <>
                  <Button size="lg" variant="destructive" onClick={stopAgent} className="gap-2">
